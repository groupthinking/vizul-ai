...
module.exports = async function handler(req, res) {
  ...
  const gatewayApiKey = process.env.AI_GATEWAY_API_KEY || process.env.AI_GATEWAY_TOKEN || process.env.VERCEL_OIDC_TOKEN;
  const gatewayModel = process.env.AI_GATEWAY_MODEL || 'openai/gpt-5.5';
  const geminiApiKey = process.env.GEMINI_API_KEY;
  if (!gatewayApiKey && !geminiApiKey) {
    return jsonResponse(res, 503, { error: 'Configure AI_GATEWAY_API_KEY (preferred) or GEMINI_API_KEY.' });
  }

  const { prompt, chartType = 'auto', tone = 'executive' } = req.body || {};
  if (!prompt || typeof prompt !== 'string' || prompt.trim().length < 3) {
    return jsonResponse(res, 400, { error: 'Please provide data or a visualization request.' });
  }
  ...
  try {
    ...
    return jsonResponse(res, 502, { error: error.message || 'Failed to generate visualization.' });
  }
};
